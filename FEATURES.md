# Features

## Core Architecture

- **Fullpage Scroll System**: GSAP-powered snap scroll transitions with scroll hijacking.
- **Atomic Sections**: Each section (`About`, `Skills`, `Projects`, etc.) is a self-contained feature component.
- **Three.js Background**: Immersive 3D interactive background integrated with React.

## Interactive Elements

- **Quantum Loader**: Custom SVG-based asset preloader with progress tracking.
- **Section Dot Navigation**: Interactive sidebar for quick jump and visual section tracking.
- **AIChatbot**: Portfolio assistant with suggested questions and chat history.
- **Voice Control**: Speech recognition for hands-free navigation commands.
- **ThreeBackground**: Dynamic 3D graphics that respond to mouse interaction.

## UI/UX Features

- **Project Showcase**: Filterable grid with category-based project organization.
- **Flip Cards**: Interactive cards revealing project details and achievements.
- **Counter-Up Stats**: Animated statistics in the About section (e.g., Performance Optimization).
- **Responsive Overflow**: Intelligent content clipping management with internal scroll containers.
- **Dynamic Headers**: Auto-highlighting navigation links based on viewport visibility.

## Testing & Quality

- **Playwright E2E**: Comprehensive test suite covering alignment, navigation, and interactive features.
- **Vibe Integration**: Chief Architect ruleset for premium aesthetics and code consistency.
- **GitHub Heatmap**: Atomic commit strategy to maximize contribution visibility.
